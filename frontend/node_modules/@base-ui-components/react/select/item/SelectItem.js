...
const SelectItem = exports.SelectItem = /*#__PURE__*/React.memo(/*#__PURE__*/React.forwardRef(function SelectItem(componentProps, forwardedRef) {
  ...
  (0, _useIsoLayoutEffect.useIsoLayoutEffect)(() => {
    if (hasRegistered) {
      if (multiple) {
        const isValueSelected = Array.isArray(rootValue) && (0, _itemEquality.itemIncludes)(rootValue, value, isItemEqualToValue);
        if (isValueSelected) {
          registerItemIndex(listItem.index);
        }
      } else if ((0, _itemEquality.compareItemEquality)(rootValue, value, isItemEqualToValue)) {
        registerItemIndex(listItem.index);
      }
    }
  ...
  function commitSelection(event) {
    if (multiple) {
      ...
      setValue(nextValue, (0, _createBaseUIEventDetails.createChangeEventDetails)('item-press', event));
    } else {
      setValue(value, (0, _createBaseUIEventDetails.createChangeEventDetails)('item-press', event));
      setOpen(false, (0, _createBaseUIEventDetails.createChangeEventDetails)('item-press', event));
    }
  }
  const defaultProps = {
    role: 'option',
