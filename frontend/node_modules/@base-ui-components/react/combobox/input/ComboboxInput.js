...
const ComboboxInput = exports.ComboboxInput = /*#__PURE__*/React.forwardRef(function ComboboxInput(componentProps, forwardedRef) {
  ...
  const setInputElement = (0, _useEventCallback.useEventCallback)(element => {
    ...
    if (hasPositionerParent && !store.state.hasInputValue) {
      store.state.setInputValue('', (0, _createBaseUIEventDetails.createChangeEventDetails)('none'));
    }
    store.apply({
  ...
  const element = (0, _useRenderElement.useRenderElement)('input', componentProps, {
    ...
    props: [inputProps, triggerProps, {
      ...
      onCompositionEnd(event) {
        ...
        store.state.setInputValue(next, (0, _createBaseUIEventDetails.createChangeEventDetails)('input-change', event.nativeEvent));
      },
      onChange(event) {
        ...
        if (isComposingRef.current) {
          ...
          if (nextVal === '' && !openOnInputClick && !hasPositionerParent) {
            store.state.setOpen(false, (0, _createBaseUIEventDetails.createChangeEventDetails)('input-clear', event.nativeEvent));
          }
          if (!readOnly && !disabled) {
            const trimmed = nextVal.trim();
            if (trimmed !== '') {
              store.state.setOpen(true, (0, _createBaseUIEventDetails.createChangeEventDetails)('input-change', event.nativeEvent));
              if (!autoHighlight) {
                store.state.setIndices({
                  activeIndex: null,
                  selectedIndex: null,
        ...
        store.state.setInputValue(event.currentTarget.value, (0, _createBaseUIEventDetails.createChangeEventDetails)('input-change', event.nativeEvent));
        const empty = event.currentTarget.value === '';
        const clearDetails = (0, _createBaseUIEventDetails.createChangeEventDetails)('input-clear', event.nativeEvent);
      ...
      onKeyDown(event) {
        ...
        if (!open && event.key === 'Escape') {
          ...
          const details = (0, _createBaseUIEventDetails.createChangeEventDetails)('escape-key', event.nativeEvent);
        ...
        if (comboboxChipsContext && event.key === 'Backspace' && event.currentTarget.value === '' && comboboxChipsContext.highlightedChipIndex === undefined && Array.isArray(selectedValue) && selectedValue.length > 0) {
          ...
          store.state.setSelectedValue(newValue, (0, _createBaseUIEventDetails.createChangeEventDetails)('none', event.nativeEvent));
          return;
        }
        ...
        if (event.key === 'Enter' && open) {
          ...
          if (activeIndex === null) {
            store.state.setOpen(false, (0, _createBaseUIEventDetails.createChangeEventDetails)('none', nativeEvent));
            return;
          }
