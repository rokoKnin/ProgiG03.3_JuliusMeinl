...
const ComboboxChip = exports.ComboboxChip = /*#__PURE__*/React.forwardRef(function ComboboxChip(componentProps, forwardedRef) {
  ...
  function handleKeyDown(event) {
    ...
    if (event.key === 'ArrowLeft') {
      ...
      store.state.setSelectedValue(selectedValue.filter((_, i) => i !== index), (0, _createBaseUIEventDetails.createChangeEventDetails)('none', event.nativeEvent));
    } else if (event.key === 'Enter' || event.key === ' ') {
      (0, _utils.stopEvent)(event);
      nextIndex = undefined;
    } else if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      (0, _utils.stopEvent)(event);
      store.state.setOpen(true, (0, _createBaseUIEventDetails.createChangeEventDetails)('list-navigation', event.nativeEvent));
      nextIndex = undefined;
    } else if (
